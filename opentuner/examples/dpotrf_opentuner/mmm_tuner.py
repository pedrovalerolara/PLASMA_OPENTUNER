...
class GccFlagsTuner(MeasurementInterface):
  ...
  def run(self, desired_result, input, limit):
    ...
    gcc_cmd += 'gcc  -c  {0}  {1}  {2}  -D{3}={4}  core_dtrsm.c  -o  core_dtrsm.o && '.format(INC,CFLAGS,LIBS,'TILE_SIZE',cfg['TILE_SIZE'])  
    gcc_cmd += 'gcc  -c  {0}  {1}  {2}  -D{3}={4}  pdtrsm.c      -o  pdtrsm.o && '.format(INC,CFLAGS,LIBS,'TILE_SIZE',cfg['TILE_SIZE'])  
    gcc_cmd += 'gcc  -c  {0}  {1}  {2}  -D{3}={4}  dtrsm.c       -o  dtrsm.o && '.format(INC,CFLAGS,LIBS,'TILE_SIZE',cfg['TILE_SIZE'])  
    ...
    gcc_cmd += 'gcc  -c  {0}  {1}  {2}  -D{3}={4}  core_dpotrf.c  -o  core_dpotrf.o && '.format(INC,CFLAGS,LIBS,'TILE_SIZE',cfg['TILE_SIZE'])  
    gcc_cmd += 'gcc  -c  {0}  {1}  {2}  -D{3}={4}  pdpotrf.c      -o  pdpotrf.o && '.format(INC,CFLAGS,LIBS,'TILE_SIZE',cfg['TILE_SIZE'])  
    gcc_cmd += 'gcc  -c  {0}  {1}  {2}  -D{3}={4}  dpotrf.c       -o  dpotrf.o && '.format(INC,CFLAGS,LIBS,'TILE_SIZE',cfg['TILE_SIZE'])  
    gcc_cmd += 'gcc  dcm2ccrb.o dccrb2cm.o core_dgemm.o pdgemm.o dgemm.o core_dsyrk.o pdsyrk.o dsyrk.o core_dtrsm.o pdtrsm.o dtrsm.o core_dpotrf.o pdpotrf.o dpotrf.o {0}  {1}  {2}  -D{3}={4}  test_opentuner.c  -o  ./tmp.bin'.format(INC,CFLAGS,LIBS,'TILE_SIZE',cfg['TILE_SIZE'])  
