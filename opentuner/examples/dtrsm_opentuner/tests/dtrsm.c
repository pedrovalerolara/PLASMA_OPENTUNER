 ...
 * @generated from compute/ztrsm.c, normal z -> d, Tue Jul 26 14:08:11 2016
 *
 **/
                 ...
                 double *B, int ldb)
{
    ...
    // Tune.
    // if (plasma_tune(PLASMA_FUNC_DTRSM, n, nrhs, 0) != PLASMA_SUCCESS) {
    ...
    for( nb = 2; nb < n-1; nb++){
        ...
        for( i = 0; i < 10; i++){ 
            ...
            #pragma omp master
            {
                ...
                if (sequence->status == PLASMA_SUCCESS) {
                    PLASMA_dtrsm_Tile_Async(side, uplo,
                                            transA, diag,
                                            alpha, &descA,
                                                   &descB,
                                            sequence, &request);
                ...
                if (sequence->status == PLASMA_SUCCESS)
                    PLASMA_dccrb2cm_Async(&descB, B, ldb, sequence, &request);
 ...
 *
 ******************************************************************************/
void PLASMA_dtrsm_Tile_Async(PLASMA_enum side, PLASMA_enum uplo,
                             PLASMA_enum transA, PLASMA_enum diag,
